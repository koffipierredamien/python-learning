...
Notre réponse tient en huit principes. Ils sont conçus pour fonctionner ensemble ; en retirer un fait tomber les autres.
...
## 2. Les huit principes directeurs (détaillés)
...
### Principe 4 — Le binôme est le troisième enseignant

Tout le travail de codage se fait **en binôme sur un seul ordinateur** (pair programming), avec deux rôles explicites :

- **Le Pilote** tient le clavier. Il est le seul à taper.
- **Le Copilote** lit la consigne, surveille les erreurs, propose. **Il n'a pas le droit de toucher le clavier ni la souris.**
- **Rotation toutes les 10 minutes**, au signal sonore de l'enseignant. Non négociable.

**Composition des binômes (fait par les enseignants, jamais par affinité) :**
- Écart d'un seul niveau maximum : un débutant total est associé à un niveau intermédiaire, **jamais** à un expert (sinon l'expert fait tout et le débutant regarde).
- Les experts sont associés entre eux ou avec des intermédiaires solides — et travaillent sur piste Noire.
- **Rotation des binômes toutes les 3 séances** pour éviter les dépendances et les conflits installés.

**Le contrat de binôme** (affiché, signé à la séance 1 — voir Annexe A5) tient en 4 phrases :
1. Je ne touche pas le clavier quand je suis copilote.
2. Je ne dis pas « t'es nul », je dis « essaie ça ».
...
2. **Je demande à mon binôme**, puis on cherche dans la **fiche mémo** de la séance.
...
**b) Le Passeport Machine.** Un petit livret de 12 gestes à valider, un tampon par geste (voir Annexe A1). Chaque geste validé est un badge. C'est valorisant, mesurable, et ça donne au grand débutant des victoires immédiates dès la séance 1, avant même de coder.
...
### Principe 7 — Motivation : grades, badges, démos

Un parcours de 20+ samedis a besoin d'un moteur. Le nôtre a quatre étages :
...
**2. Les badges (actions ponctuelles, distribués généreusement).** Ils récompensent le comportement d'apprenant, pas la performance :
`Chasseur de bug` · `Bon copilote` · `Sauveteur` (a aidé un autre binôme) · `Traducteur d'erreur` (a expliqué un message d'erreur) · `Zéro indentation ratée` · `Question en or` · `Persévérant` (a réessayé 5 fois) · `Testeur impitoyable` · `Beau design` · `Premier à finir la piste Noire`

**3. Le Mur de Mission (progression collective).** Une grande affiche : la carte du projet XO en 22 étapes, sous forme de parcours (fusée, montagne ou carte au trésor). À chaque fin de séance, **la classe entière** avance d'une case. C'est collectif : la classe réussit ou échoue ensemble, ce qui pousse les rapides à aider au lieu de fuir en avant.

**4. Les grands rendez-vous.** Ils donnent une raison d'aller jusqu'au bout :
- **Séance 8 — La première démo** : le jeu tourne en console, on y joue à deux.
- **Séance 14 — Le concours de design** : chacun personnalise son plateau.
- **Séance 18 — La Nuit du bug** : les binômes s'échangent des jeux volontairement cassés à réparer, en équipes.
...
### Principe 8 — Évaluation par la preuve, jamais par la note
...
2. **Je sais l'expliquer** — il explique à son binôme, en français, ce que fait la ligne (30 secondes). *C'est le filtre anti-copie.*
3. **Je sais le refaire** — il réussit un micro-défi de la même famille sur une variante (2 minutes), lors d'une séance ultérieure.

L'auto-évaluation de fin de séance tient en une phrase, sur un post-it ou dans le journal de bord : **« Aujourd'hui j'ai compris ___ , et je ne suis pas encore sûr de ___ . »** Les enseignants lisent ces post-it à la fin : c'est le meilleur outil de préparation de la séance suivante qui existe.
...
|---|---|---|
| **P0 — Découvreur** | Ne maîtrise pas la machine | Passeport Machine, piste Bleue, squelette de code fourni, binôme P1, vérification visuelle 3×/séance |
...
| Une machine tombe en panne | Le binôme continue sur l'autre machine (déjà en pair programming : impact quasi nul). Une machine de réserve allumée en fond de salle. |
...
- **E. Relationnel & émotionnel** : binôme conflictuel, élève moqué, timide qui n'ose pas demander, leader qui monopolise, démotivation, peur de l'erreur, crise de frustration devant un bug.
...
| Post-it « je ne suis pas sûr de » sur la même notion | < 30 % du groupe | La notion est reprise en ouverture de la séance suivante |
...
- **Aucun devoir obligatoire.** Tous les élèves n'ont pas d'ordinateur à la maison ; rendre le travail à la maison obligatoire créerait une inégalité immédiate et durable. Les défis maison sont des bonus, valorisés par des badges, jamais requis pour suivre.
...
|---|---|---|---|
| **0. Décollage** | 1 | Test de Décollage, Passeport Machine, premier `print()`, premier message affiché | environnement, exécuter un programme |
| **1. Parler au joueur** | 2–5 | Le jeu dit bonjour, demande les noms, affiche un plateau vide | variables, `input()`, `print()`, chaînes, types |
| **2. Le plateau vit** | 6–9 | On place un X ou un O, on rejoue, on alterne les joueurs | listes, indices, conditions, boucles — **1ʳᵉ démo jouable en S8** |
...
| **4. L'interface graphique** | 14–18 | Fenêtre, grille cliquable, symboles dessinés, personnalisation, rejouer | Tkinter, événements, callbacks — **concours de design en S14, Nuit du bug en S18** |
| **5. Finition & intelligence** | 19–22 | Score, sons, écran de victoire, **IA de l'ordinateur** (Jeremiah), distribution du jeu | organisation du code, algorithme de choix, packaging — **Tournoi final** |
...
- **[A1 — Test de Décollage & Passeport Machine](annexes/A1-diagnostic-et-passeport.md)** — les 3 stations, la grille d'observation, les 12 gestes du passeport
...
- **[A4 — Grades, badges et Mur de Mission](annexes/A4-grades-et-badges.md)** — le système de motivation complet
- **[A5 — Outils imprimables](annexes/A5-outils-imprimables.md)** — cartes de signalisation, contrat de binôme, règle des 3 avant moi, journal de bord, grille de test, tableau de suivi
